...
async function sendWelcome(email, firstName) {
  ...
  const dealsHtml = (deals || []).map(d => `
    <tr>
      ...
      <td style="padding:8px">
        <a href="${d.amazon_url||'#'}" style="background:#FF5E1A;color:#fff;padding:6px 14px;text-decoration:none;border-radius:3px;font-size:13px;white-space:nowrap">View Deal</a>
      </td>
    </tr>`).join('');

  const fbRow = FB_LINK
    ? `<a href="${FB_LINK}" style="display:inline-block;margin-left:10px;background:#1877F2;color:#fff;font-weight:700;padding:10px 22px;border-radius:4px;text-decoration:none">Join Facebook Group</a>`
    : '';
  ...
  await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [email],
      subject: `${name !== 'there' ? name + ', you' : 'You'}'re in — here are today's top deals`,
      html: `
        <div style="font-family:sans-serif;max-width:560px;margin:0 auto;background:#111114;color:#fff;padding:32px;border-radius:8px">
          <h2 style="color:#FF5E1A;margin:0 0 8px">You're confirmed!</h2>
          <p style="color:#ccc">Hi ${name}, welcome to ${FROM_NAME}. You'll now get daily alerts on the biggest Amazon deals.</p>

          ${dealsHtml ? `
          <h3 style="color:#fff;margin:28px 0 12px">Today's top picks</h3>
          <table width="100%" cellpadding="0" cellspacing="0" style="background:#1a1a1f;border-radius:6px">
            ${dealsHtml}
          </table>
          <p style="margin:16px 0"><a href="${SITE_URL}/top-deals" style="color:#FF5E1A">Browse all deals &rarr;</a></p>
          ` : ''}

          <h3 style="color:#fff;margin:28px 0 12px">Get instant alerts</h3>
          <p style="color:#aaa;font-size:14px">Join our communities for real-time deal drops.</p>
...
module.exports = async function handler(req, res) {
  try {
    ...
    if (!sub) {
      return redirect(res, `${SITE_URL}/?confirmed=notfound`);
    }
    ...
    if (!sub.confirmed) {
      ...
      // Await welcome email before redirecting — serverless freezes on response
      await sendWelcome(email, sub.first_name).catch(console.error);
    }

    return redirect(res, `${SITE_URL}/welcome`);

  } catch (err) {
    console.error('confirm handler error:', err);
    return res.status(500).send('<p>Something went wrong. Please try the link again.</p>');
  }
};
